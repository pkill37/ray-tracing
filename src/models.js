...
function centroidRefinement(coordsArray, colorsArray, recursionDepth) {
    ...
    // Iterate through the original triangular faces
    for(var origIndex = 0; origIndex < origArrayLength; origIndex += 9) {
        // Call the recursive subdivision function
...
function moveToSphericalSurface( coordsArray ) {
    ...
    for (var i = 0; i < coordsArray.length; i += 3) {
        var tmp = normalize(coordsArray.slice(i, i+3));
        coordsArray[i] = tmp[0];
        coordsArray[i+1] = tmp[1];
        coordsArray[i+2] = tmp[2];
    }
}
...
function computeVertexNormals( coordsArray) {
    ...
    for(var i = 0; i < coordsArray.length; i += 9) {
        // Compute unit normal vector for each triangle
        var normalVector = computeNormalVector(
            coordsArray.slice(i, i + 3),
            coordsArray.slice(i + 3, i + 6),
            coordsArray.slice(i + 6, i + 9)
        );
